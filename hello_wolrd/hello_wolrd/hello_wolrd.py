...
class AuthorForm(
    ...
    _fields=(("Name"          , "name"           ),
             ...
             ("Died Date"     , "died_dated"     ),
             )
    def on_apply(SELF, mgr):
        mgr.next(display())
    def query():
        with rx.session() as session:
            return session.exec(Author.select()).scalars().all()
    ...
    def on_apply(SELF, mgr):
        with rx.session as session:
            author_old=          session.exec(
                Author
                .select()
                .where(Author.died_date == SELF.died_date
                ).where( Author.id == SELF.id)
            )
            if not author_old:
                raise ValueError("The died_date is not in the database or it does not belong to this book")
            else:
                for a in author_old:
                    a.died_date = SELF.died_date
...
def action(self, mgr):
    with rx.session as session:
        ...
        books={}
        for book in books:
            books[book.title]=book.registry
        return books
    books=display()
    return books
\
...
def tableRow(Book) ->rx.Component:
    ...
    return rx.tr(
        rx.td(
            ...
            cursor="pointer"
            ).on_click(lambda _: query.set_active_book(Book)
        )
        ,
        rx.td(
            rx.text(
